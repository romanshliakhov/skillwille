(() => {
    ...
    class Quiz {
        ...
        updateQuizState() {
            ...
            if (this.currentStep === 0) this.quiz.classList.add("_quiz-min");
            if (this.currentStep === this.totalSteps - 1) this.quiz.classList.add("_quiz-max");
        }
        updateProgress() {
            const progress = this.currentStep === 0 ? 0 : this.currentStep / this.totalSteps * 100;
            if (this.progressLine) this.progressLine.style.setProperty("--progress-quiz", `${progress}%`);
            if (this.stepCounter) this.stepCounter.style.setProperty("--progress-quiz-index", `'${this.currentStep + 1}'`);
        }
        updateTagsCompletion() {
            this.steps.forEach((step, index) => {
                const matchingTag = this.tags[index];
                if (matchingTag) if (index < this.currentStep) matchingTag.classList.add("_complete"); else matchingTag.classList.remove("_complete");
            });
        }
        showError(step) {
        ...
        nextStep() {
            ...
            if (this.currentStep === this.totalSteps - 1) {
                ...
                if (popupSelector && typeof flsModules !== "undefined" && flsModules.popup) flsModules.popup.open(popupSelector);
                return;
            }
